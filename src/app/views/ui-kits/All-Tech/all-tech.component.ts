...
@Component({
  ...
  styleUrls: ['./all-tech.component.scss'],
  providers: [DatePipe]
...
export class AllTechComponent implements OnInit {
  ...
  onclick() {
    this.LocalStoreService.alltechnician().subscribe(data => {
      ...
      console.log("the data of the all",this.alltechnician)
  ...
  opentechniciandata() {
    ...
    // this.adminservice.gettechnician_jobcard(this.techspecificdata, startdate, enddate, status).subscribe(data => {
      this.LocalStoreService.get_alltech_jobcard(this.alltechids, startdate, enddate, status).subscribe(data =>{
      this.totaltechdata = data.response
